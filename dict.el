...
;;; Commentary:
;;;
;;; A dictionary consists of a list containing either 5 or 10
;;; elements, storing the following data (see the dict-create function
;;; for details):
;;;
;;; 'DICT
;;; filename
...
;;; lookup-only
;;; lookup-hash
;;; --- rest only if dictionary is not lookup-only ---
;;; lookup-speed
;;; complete-hash
;;; complete-speed
;;; ordered-hash
...
;;; ================================================================
;;;  Internal functions for use in the dictionary package
...
(defmacro dic-set-lookup-hash (dict hash)  ; INTERNAL USE ONLY
  ...
  `(setcar (cdr (cdr (cdr (cdr ,dict)))) ,hash)
)
...
(defmacro dic-set-completion-hash (dict hash)  ; INTERNAL USE ONLY
  ;; Set the completion hash for dictionary DICT
  `(setcar (cdr (cdr (cdr (cdr (cdr (cdr ,dict)))))) ,hash)
)
...
(defmacro dic-set-ordered-hash (dict hash)  ; INTERNAL USE ONLY
  ;; Set the completion hash for dictionary DICT
  `(setcar (cdr (cdr (cdr (cdr (cdr (cdr (cdr (cdr ,dict)))))))) ,hash)
)
...
(defmacro cache-create (vect maxnum)
  ;; Return a completion cache entry
  `(cons ,vect ,maxnum)
)


(defmacro cache-vect  (cache)
  ;; Return the completions vector for cache entry CACHE
  `(car ,cache)
)


(defmacro cache-num (cache)
...
;;; ===============================================================
;;;     Utility functions not directly related to dictionaries
...
(defun dict-create (&optional filename
...
saving the dictionary. If the variable `dict-auto-save' is set, then the
dictionary will be included when saving dictionaries periodically or on
exiting emacs.
...
is stored!\)"
  ...
  ;;  ordered-speed)
  (if lookup-only
      (list 'DICT filename insert-function t (make-hash-table :test 'equal))
    (list 'DICT filename
...
(defun dict-create-type (name type &optional filename lookup-speed
			      complete-speed ordered-speed)
...
\"greater-than\" comparison of the data."
  ...
  (let (insfun rankfun lookup-only)
    ...
    (dict-create filename lookup-speed complete-speed ordered-speed
...
(defun dict-insert (dict word &optional data insert-function)
  "Insert WORD and DATA into dictionary DICT. If WORD does not already exist,
this creates it. How the data is inserted depends on the dictionary's
insertion function (see `dict-create').
...
with WORD in the dictionary (or nil if none already exists). It should return
the data to insert."
...
returned."
  ...
  (let* ((dictlist (if (dict-p dict) (list dict) dict)) dic
	...
	(if (and cache (or (null (setq cachenum (cache-num cache)))
	  ...
	  ;; completion speed, add the results to the completion hash
	  (when (and (setq speed (dic-completion-speed dic))
		     (or (eq speed t) (> time speed)))
	    (puthash str (cache-create vect maxnum) (dic-completion-hash dic))
	  )
	)
...
that has side-effects."
  ...
  (save-excursion
	...
	(when (= 0 (mod lines 2))
	  ...
	  (when (setq entry (dict-read-line))
	    (dict-insert dict (car entry) (cdr entry))))
	(message "Populated dictionary with %d words" lines)
      )
      (kill-buffer buff)
    )
  )
...
different file."
  ...
  ;; construct the dictionary name from the filename
  (let* ((dict (eval dict))
	 (filename (dic-filename dict)) dictname
    ...
    (unless (and filename (> (length filename) 0))
      (setq filename (read-file-name "No file associated with dictionary.\
 File to save to: "))
    ...
    ;; remove .elc extension from filename to give dictionary name
    ...
    (save-excursion
	  ...
	  (progn
	    ...
	    (setq tmpdict (list 'DICT filename (dic-insfun dict)
				lookup-only lookup-alist))
	  )
	...
	;; create the completion alist, if necessaru
	...
	;; create the ordered completion alist, if necessaru
	(when ordered-speed
	...
	;; generate the structure to save
	(setq tmpdict (list 'DICT filename
      ...
      (when (> (length hashcode) 0) (insert "(require 'dict)\n"))
	...
	(error "Error saving dictionary %s" dictname))
    )
  )
)
...
otherwise."
  ...
  (let (dictname dict)
    ...
    (when (not (dict-p dict))
      (beep)
      (error "Error loading dictionary %s" dictname))
    
    ;; ensure the file name associated with the dictionary matches the file it
    ;; was loaded from
